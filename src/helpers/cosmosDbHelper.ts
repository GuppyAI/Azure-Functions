import { Container, CosmosClient, Database, ResourceResponse } from "@azure/cosmos";
...
export class CosmosDbHelper {
    ...
    constructor(private readonly connectionString: string, private readonly databaseName: string, private readonly containerName: string) {
    ...
    public async saveMessage(userID: string, content: string, role: ChatCompletionRole): Promise<Array<ChatCompletionMessageParam>> {
        const hashedUserID = hashUserID.hashUserID(userID);
        if (!await this.userExists(userID)) {
            ...
            return ((response.resource.messages as unknown) as Array<ChatCompletionMessageParam>)
        }
        else {
            const response = await this.container.item(hashedUserID, hashedUserID).patch({
                operations: [{
                    op: "add",
                    path: "/messages/-",
                    value: { role, content }
    ...
    public async deleteMessages(userID: string): Promise<void> {
        const hashedUserID = hashUserID.hashUserID(userID);
        if (await this.userExists(userID)) {
            await this.container.item(hashedUserID, hashedUserID).replace({ id: hashedUserID, messages: [] });
        }
    }
...
class hashUserID {
    ...
    public static hashUserID(userID: string): string {
        if (!hashUserID.hashedUserID) {
            hashUserID.hashedUserID = createHash("sha256").update(userID).digest("hex");
        }
